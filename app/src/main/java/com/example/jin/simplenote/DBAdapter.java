...
public class DBAdapter extends SQLiteOpenHelper {
    ...
    // create
    public boolean insertInfo(String meeting, String time, String rate) {
    ...
    public ArrayList<Info> getAllInfo() {
        ...
        Cursor c = db.query(TABLE_NAME, new String[] {ID, MEETING, TIME, RATE}, null, null, null, null, ID + " DESC");
        ...
        if (c.moveToFirst()) {
            ...
            final int indexRate = c.getColumnIndex(RATE);

            do {
                int id = c.getInt(indexId);
                String meeting = c.getString(indexMeeting);
                String time = c.getString(indexTime);
                String rate = c.getString(indexRate);
                info.add(new Info(id, meeting, time, rate));
            } while (c.moveToNext());
        }
        c.close();
